...
if __name__ == '__main__':
    ...
    # plt.rc('xtick', labelsize=f_size)
    # plt.rc('ytick', labelsize=f_size)
    setup_folder = "test_samplers/RO_NF_STD2"
    ...
    # case_nums = np.arange(10)
    case_nums = np.arange(0, 10)
    case_dirs = [f"{setup_folder}/seed{i}" for i in case_nums]
    # case_dirs = [f"{setup_folder}/test_{i}" for i in case_nums]
    ...
    xlim = (-60, 190)
    ylims = np.array([xlim]*len(case_nums))
    ...
    fig_scale = 2
    scatter_fig = plt.figure(figsize=(fig_scale*len(case_dirs),fig_scale*len(scatter_folders)))
    gs = scatter_fig.add_gridspec(len(scatter_folders),len(case_dirs), #width_ratios=[1]*len(scatter_folders),height_ratios = y_lens,
    ...
    for k, case_dir in enumerate(case_dirs):
        ...
        if plot_scatter:
            for i, folder in enumerate(scatter_folders):
                ...
                step = final_step
                ax = axs[i,k]
                if folder == gtsam_folder or folder == nfisam_folder:
                ...
                # if i == 0:
                ax.set_xlabel(f"Case {k+1}", fontsize=f_size + 4)
            ...
            for ax in axs.flat:
                ax.label_outer()
            scatter_fig.suptitle(f'Time step {step}', fontsize=f_size+4,y=0.91)
            if all_scatter:
                scatter_fig.savefig(f"{plot_dir}/all_scatter_final_step{step}.png", dpi=300, bbox_inches="tight")
            else:
                scatter_fig.savefig(f"{plot_dir}/scatter_final_step{step}.png", dpi=300, bbox_inches="tight")
        # translation dim
        ...
        if compute_traj_div_perf:
            ...
            step = final_step
            ref_dir = f"{case_dir}/{gtsam_folder}"
            ref_sample_file = f"{ref_dir}/step{step}"
            ...
            var2mean = getMeans(ref_order, f"{ref_dir}/step_{step}_marginal")
            ...
            # else:
            #     print(f"{gtsam_folder} has fewer samples than others at step {step}.")
            #
            ...
            mean_xy = []
            for var in ref_order:
                mean_xy += [var2mean[var].x,var2mean[var].y]
            diff_xy = mean_xy - true_xy
            rmse = np.sqrt(np.mean(diff_xy**2))
            time_data = folder2timing[gtsam_folder]
            # Case, Step, Solver, Time, RMSE
            case_all_step_data.append([k, step, folder2lgd[gtsam_folder], time_data[step], rmse])
            for i, folder in enumerate(baseline_folders):
                dir = f"{case_dir}/{folder}"
                ...
                rmse = np.sqrt(np.mean(diff_xy**2))
                last_rmse = rmse
                case_all_step_data.append([k, step, folder2lgd[folder], folder2timing[folder][step], rmse])
    columns = ["Case", "Step", "Solver", "Time", "RMSE"]
    dd_df = None
    if compute_traj_div_perf:
    ...
    if dd_df is not None and plot_perf:
        ...
        for j, target in enumerate(target_names):
            ...
            ax.set(yscale="log")
            # if target == "RMSE":
            #     ax=sns.lineplot(ax=ax, data=tmp_data, x="Case", y=target, label=folder2lgd[folder],color=solver2color[folder2lgd[folder]],
            #               # marker=solver2marker[folder2lgd[folder]],
            #               linestyle=folder2linestyle[folder],
            #               markersize=int(m_s/4))
            # else:
            #     ax=sns.lineplot(ax=ax, data=tmp_data, x="Case", y=target,color=solver2color[folder2lgd[folder]],
            #               # marker=solver2marker[folder2lgd[folder]],
            #               linestyle=folder2linestyle[folder],
            #               markersize=int(m_s/4))
